...
def check_password_simple():
    """Simple password check - second layer of security"""
    ...
    if not st.session_state.password_correct:
        ...
        with col2:
            st.markdown("# 🔒 Merrimac Portfolio Analytics")
            st.markdown("### Please enter password to access the dashboard")
            st.markdown("---")
            ...
            st.markdown("---")
            st.caption("🔐 This application is protected by two-factor authentication")
...
def main():
    ...
    # ⭐ ADD THIS PASSWORD CHECK FIRST ⭐
    if not check_password_simple():
        st.stop()  # Stop execution if password not entered
